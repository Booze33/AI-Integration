...
export type ChatTransport = 'none' | 'websocket' | 'sse';
...
function getChatWebSocketUrl(): string {
  const base = appConfig.apiUrl.trim();
  if (base.startsWith('https://')) {
    return `${base.replace(/^https:\/\//, 'wss://')}${WS_CHAT_PATH}`;
  }
  if (base.startsWith('http://')) {
    return `${base.replace(/^http:\/\//, 'ws://')}${WS_CHAT_PATH}`;
  }
  return `${base}${WS_CHAT_PATH}`;
}
...
export function useChatStream(options: UseChatStreamOptions = {}) {
  ...
  const [transport, setTransport] = useState<ChatTransport>('none');
  ...
  const start = useCallback(
    async (messages: ChatStreamMessage[], sessionId?: string) => {
      abort();
      ...
      const startViaWebSocket = async (): Promise<boolean> => {
        ...
        return new Promise<boolean>((resolve, reject) => {
          ...
          ws.onmessage = (event) => {
            let payload: WebSocketEventPayload;
            try {
              payload = JSON.parse(event.data as string) as WebSocketEventPayload;
            } catch {
              return;
            }
            ...
            if (eventType === 'start') {
              const nextStreamId = payload.streamId || payload.id;
              if (nextStreamId) {
              ...
              if (options.onStart) {
                void options.onStart(payload as SSEEventPayload);
              }
              return;
            }
            ...
            if (eventType === 'chunk' && payload.content) {
              if (options.onChunk) {
                void options.onChunk(payload.content);
              }
              return;
            }
            ...
            if (eventType === 'done') {
              completed = true;
              if (options.onDone) {
                void options.onDone(payload as SSEEventPayload);
              }
              ws.close();
              finish(true);
              return;
            ...
            if (eventType === 'error') {
              completed = true;
              const errorMessage = payload.message || 'Stream error';
              setError(errorMessage);
              if (options.onError) {
                void options.onError(errorMessage, payload as SSEEventPayload);
              }
              ws.close();
