...
export function printEnvConfig(): void {
  ...
  const safeEnv = {
    ...
    JWT_PRIVATE_KEY: env.JWT_PRIVATE_KEY ? maskSensitiveValue(env.JWT_PRIVATE_KEY) : 'not-set',
    JWT_PUBLIC_KEY: env.JWT_PUBLIC_KEY ? maskSensitiveValue(env.JWT_PUBLIC_KEY) : 'not-set',
    TENANT_CONFIG_ENCRYPTION_KEY: maskSensitiveValue(env.TENANT_CONFIG_ENCRYPTION_KEY),
    AI_API_KEY: env.AI_API_KEY ? maskSensitiveValue(env.AI_API_KEY) : 'not-set',
